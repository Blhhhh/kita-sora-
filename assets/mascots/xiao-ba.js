...
(() => {
  ...
  const assetUrl = new URL('xiao-ba-3d.png', currentScript.src).href;
  const style = document.createElement('style');
  style.textContent = `
    .dynamic-island{position:fixed;right:clamp(18px,3vw,42px);bottom:clamp(16px,3vw,36px);z-index:2000;width:clamp(150px,15vw,218px);aspect-ratio:2/3;border:0;padding:0;background:none;cursor:pointer;isolation:isolate;filter:drop-shadow(0 14px 24px rgba(31,52,75,.19));animation:ks-xiaoba-float 5.8s ease-in-out infinite;transition:filter .3s ease}
    .dynamic-island:hover{filter:drop-shadow(0 18px 30px rgba(31,52,75,.25));animation-play-state:paused}
    ...
    .dynamic-island__eye{position:absolute;left:23.2%;top:40.2%;width:5.2%;height:3.25%;border-radius:50%;background:#8c4f2d;opacity:0;transform:scaleY(.05);transform-origin:center;pointer-events:none}
    .dynamic-island.is-blinking .dynamic-island__eye{animation:ks-xiaoba-blink .42s ease-in-out 1}
    @keyframes ks-xiaoba-float{0%,100%{transform:translate3d(0,0,0) rotate(-1.5deg)}50%{transform:translate3d(-4px,-13px,0) rotate(1.5deg)}}
    ...
    @media(prefers-reduced-motion:reduce){.dynamic-island{animation:none}.dynamic-island.is-blinking .dynamic-island__eye{animation:none;opacity:1;transform:scaleY(.7)}}
  `;
